...
void ImGui::NewFrame()
{
    ...
    g.BackgroundDrawList.PushTexture(g.IO.Fonts->Texture);
    g.BackgroundDrawList.PushClipRectFullScreen();

    g.ForegroundDrawList.Clear();
    g.ForegroundDrawList.PushTexture(g.IO.Fonts->Texture);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            int elem_offset = 0;
            for (const ImDrawCmd* pcmd = &draw_list->CmdBuffer.front(); pcmd < &draw_list->CmdBuffer.back(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                char buf[300];
                ImFormatString(buf, IM_ARRAYSIZE(buf), "Draw %4d triangles, tex %d, clip_rect (%4.0f,%4.0f)-(%4.0f,%4.0f)",
                    pcmd->ElemCount/3, pcmd->Texture.GetID(), pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
                bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - &draw_list->CmdBuffer.front()), "%s", buf);
